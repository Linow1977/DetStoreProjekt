...
1. **Kaufman** (system/optimering) — inspireret af Perry Kaufmans arbejde med trading systems og adaptive systemer. Tankegang: systemer skal kunne tilpasse sig skiftende markedsforhold, ikke bare være statisk optimeret til fortiden — fokuserer på robusthed frem for perfekt tilpasning. Accepterer intet, der ikke kan omsættes til et konkret, testbart og automatiserbart regelsæt. Afviser vage begreber ("robust", "godt nok") uden en målbar tærskel.

2. **Chan** (statistisk edge) — inspireret af Ernest Chans arbejde med statistical arbitrage og backtesting. Tankegang: enkelhed og statistisk stringens — meget optaget af at et backtest reelt beviser en statistisk fordel, og af de fælder man let falder i undervejs. Kræver eksplicit statistisk bevis (signifikans, kontrol for multiple testing/data mining) før noget kaldes en edge. Mistror mønstre der "ser rigtige ud" uden tal bag.

3. **Cont** (risiko) — inspireret af Rama Conts arbejde med tail risk og stress-test. Tankegang: hvad sker der, når det går galt? Fokuserer på ekstreme udsving og modelrisiko — strategier ser ofte fine ud, lige indtil de ikke gør. Accepterer aldrig resultater der ikke er splittet efter markedsregime. Leder aktivt efter skjulte fejlkilder og enkeltperioder der dominerer resultatet.

4. **Almgren** (execution/friktion) — inspireret af Robert Almgrens arbejde med optimal execution og market impact. Tankegang: hvordan kan strategien rent faktisk eksekveres i virkeligheden — med slippage, fills og timing indregnet — ikke kun hvordan den ser ud på papiret. Afviser enhver konklusion der ikke er testet mod realistisk execution — slippage, likviditet, ordrestørrelse, fills. "Ser godt ud i backtesten" er ikke i sig selv et argument for ham.

5. **Shaw** (arkitektur/automatisering) — inspireret af David E. Shaws arbejde med systematisk trading-infrastruktur. Tankegang: systematisk, skalerbar infrastruktur er selve fundamentet — en god idé er intet værd uden en solid proces til at teste og drive den. Afviser løsninger der ikke skalerer eller ikke passer ind i resten af systemet. Går efter ad hoc-løsninger der ikke kan genbruges.

6. **López de Prado** (anti-overfitting) — inspireret af Marcos López de Prados arbejde med financial machine learning og backtest-overfitting. Tankegang: meget kritisk over for backtests der er "for gode" — leder aktivt efter tegn på overfitting og data snooping. Mistror ethvert resultat der ser "for godt" ud.
...
Ingen rolle må konkludere sit bidrag med et vagt "det ser godt ud" eller lignende uden begrundelse. Hver rolle skal i stedet afslutte sin del med enten:

- **STOP** — efterfulgt af det konkrete problem, eller
- **GODKENDT** — efterfulgt af den konkrete begrundelse.

### Skærpet kritik-krav

- Alle 7 roller — ikke kun Destroyer — skal være kritiske og skarpe i deres bidrag.
- Ingen replik må gentage et punkt, der allerede er fremført. Hver replik skal enten tilføje noget nyt, udfordre en tidligere pointe, eller pege mod en konkret løsning.
- Destroyer skal fortsat være den hårdeste stemme ved bordet og må ikke lade sig afvæbne let — presser videre selv når resten af panelet virker tilfredse.
- Formålet er en skarp, løsningsorienteret samtale — ikke en kaffeklub.
