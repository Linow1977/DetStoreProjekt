# Hjaelpefunktioner til at aflaese Output-panelet i TradeStation Development
# Environment (TSDev.exe): antal linjer, toem panelet og laes al teksten,
# samt finde og markere en raekke i en liste (Open-dialogen).
...
public class TsDev {
  ...
  // Markerer en raekke i en liste i TSDev (fx Open-dialogens liste). Kraever
  // ligesom ReadAll kryds-proces hukommelse til LVITEM-strukturen.
  public static bool SelectRow(IntPtr lv, int row) {
    const uint LVM_SETITEMSTATE = 0x102B;
    const uint LVM_ENSUREVISIBLE = 0x1013;
    uint pid; GetWindowThreadProcessId(lv, out pid);
    IntPtr proc = OpenProcess(PROCESS_ALL, false, pid);
    if (proc == IntPtr.Zero) return false;
    IntPtr remote = VirtualAllocEx(proc, IntPtr.Zero, 64, MEM_COMMIT | MEM_RESERVE, PAGE_RW);
    try {
      byte[] item = new byte[60];
      // 32-bit LVITEM: mask(0) iItem(4) iSubItem(8) state(12) stateMask(16)
      BitConverter.GetBytes((uint)0x8).CopyTo(item, 0);    // LVIF_STATE
      BitConverter.GetBytes(row).CopyTo(item, 4);
      BitConverter.GetBytes((uint)0x3).CopyTo(item, 12);   // LVIS_FOCUSED | LVIS_SELECTED
      BitConverter.GetBytes((uint)0x3).CopyTo(item, 16);
      uint w;
      WriteProcessMemory(proc, remote, item, (uint)item.Length, out w);
      SendMessage(lv, LVM_SETITEMSTATE, (IntPtr)row, remote);
      SendMessage(lv, LVM_ENSUREVISIBLE, (IntPtr)row, IntPtr.Zero);
      return true;
    } finally {
      VirtualFreeEx(proc, remote, 0, MEM_RELEASE);
      CloseHandle(proc);
  ...
  // Raekkenummeret hvor foerste kolonne er praecis 'tekst'. -1 hvis den ikke findes.
  public static int FindRow(IntPtr lv, string tekst) {
    var rows = ReadAll(lv);
    for (int r = 0; r < rows.Count; r++) if (rows[r][0] == tekst) return r;
    return -1;
  }
